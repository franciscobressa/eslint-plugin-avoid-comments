module.exports = {
  ...
  create(context) {
    ...
    const isConsecutiveComment = comment => {
      ...
      return previousTokenOrComment && !isLineBeforeEmpty && ['Block', 'Line'].includes(previousTokenOrComment.type)
    }

    const isAllowedComment = (comment, allowedKeywords) => {
      const startsWithAllowedKeyword = allowedKeywords.some(keyword => comment.value.trim().startsWith(keyword))
    ...
    return {
      Program() {
        ...
        comments.forEach(comment => reportNonAllowedComment(comment, allowedKeywords))
      },
    }
  },
}
